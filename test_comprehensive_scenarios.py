...
ソルバーを実行し、各シナリオごとに重点チェック項目(希望休・有休確定の
...
シナリオ:
  ...
  B. 希望休・有休集中シナリオ - 特定週末・特定店舗への希望休/有休確定の集中
...
# ---------------------------------------------------------------------------
# 共通チェック(4シナリオ共通で使い回す)。
# 【重点チェック項目1】希望休・有休確定の反映精度
# ---------------------------------------------------------------------------

def check_confirmed_leave_zero_attendance(shift_df: pd.DataFrame, requests_df: pd.DataFrame) -> list[str]:
    """「有休確定」が設定された日は、例外なく全スタッフが店舗出勤ゼロになっているか。"""
    issues = []
    if requests_df.empty:
        return issues
    confirmed = requests_df[requests_df["kind"] == "有休確定"]
    ...
    for _, r in confirmed.iterrows():
        if (r["staff_id"], r["date"]) in working_pairs:
            issues.append(f"{r['name']} {r['date']}: 有休確定日にもかかわらず出勤している")
    return issues
...
def check_salaried_workdays_exact(
    ...
    """店長・正社員の出勤日数が「基準出勤日数 − 期間内の有休確定日数」に厳格一致しているか。

    基準出勤日数そのものは対象期間のカレンダーから動的に導出する(固定値を
    ハードコードしない)。有休確定を1件も持たないスタッフは「基準出勤日数と
    厳格一致」、有休確定をn件持つスタッフは「基準出勤日数-n日」を期待値とする。
    """
    issues = []
    ...
    for _, row in salaried.iterrows():
        ...
        if cnt != expected:
            issues.append(
                f"{row['name']}: 出勤{cnt}日"
                f"(期待値{expected}日 = 基準{expected_workdays}日-有休確定{confirmed_cnt}日)"
            )
    return issues
...
def check_shortage_attribution_accuracy(shortages: list[dict]) -> list[str]:
    """負荷・エッジケースシナリオ(希望休・有休確定の意図的な集中)では、店舗の
...
def run_common_post_solve_checks(
    ...
    record(scenario, "有休確定日の出勤ゼロ", check_confirmed_leave_zero_attendance(result.shift_df, requests_df))
    record(
        scenario,
        "店長・正社員の出勤日数(基準日数-有休確定日数に厳格一致)",
    ...
    if strict_store_headcount:
        ...
        # 希望休・有休確定を意図的に集中させた負荷シナリオでは、週末必要人員
...
def scenario_a_normal_month() -> None:
    ...
    # 正社員の有休確定は平日に限定する(土日祝は週末必要人員14枠(7店舗×2名)
    # に対し社員・嘱託の総数もちょうど14名という無遊び構成のため、土日祝の
    # 有休確定はどのスタッフでも構造的に不足を生みやすい=シナリオBで別途
    ...
    requests_rows = [
        _req(name_to_id, "生駒", weekday_business_days[3], "有休確定"),
    ...
    if slot_days:
        requests_rows.append(_req(name_to_id, "田中", slot_days[0], "有休申請"))
...
def scenario_b_concentrated_leave() -> None:
    ...
    # 中川店・徳重店・稲沢店それぞれの主力スタッフの希望休・有休確定が
    # 同一週末に重複・集中するケース(=各店の主戦力が同時に抜ける負荷シナリオ)。
    requests_rows = [
        _req(name_to_id, "内田", w1, "有休確定"),   # 名古屋中川店長
        _req(name_to_id, "真田", w1, "希望休"),      # 正社員(全店ヘルプ可)
        _req(name_to_id, "加藤", w1, "有休確定"),    # 徳重店長
        _req(name_to_id, "生駒", w2, "有休確定"),    # 稲沢店長
        _req(name_to_id, "山岡", w2, "希望休"),      # 嘱託(稲沢主所属)
    ]
    if holiday_only_days:
        h1 = holiday_only_days[0]
        requests_rows.append(_req(name_to_id, "小林", h1, "有休確定"))  # 新蟹江店長
        requests_rows.append(_req(name_to_id, "竹内", h1, "有休申請"))  # 嘱託(全店ヘルプ可)
...
def scenario_d_future_month_switching() -> None:
    ...
    try:
        ...
        utils.append_kyuka_request("辻本", dt.date(2026, 10, 5), "有休確定", utils.kyuka_log_path_for(2026, 10))
        at.run()
        if list(at.exception):
