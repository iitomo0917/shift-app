...
W_YAMAOKA_TAKEUCHI_KANIE_SWAP = 20  # 山岡が新蟹江店に配属され、かつ竹内が同日に
                        # 新蟹江店以外の店舗(竹内は全7店舗に出勤可能)に配属されて
                        # いる組み合わせへの軽いペナルティ。竹内を新蟹江店へ、
                        # 山岡を竹内がいた店舗へ入れ替えたいとの要望(2026/9時点、
                        # 当初は徳重店・極楽店のみが対象だったが、10/24の実例(竹内
                        # =天白植田店)を受けて竹内の勤務可能店舗全体に拡大)による。
                        # 他店舗の人員不足解消などでどうしても入れ替えられない場合は、
...
def solve_shift(
    ...
    # --- 4g. 山岡(新蟹江店)×竹内(新蟹江店以外)の入れ替え推奨(ソフト制約) -----------
    #     山岡が新蟹江店に配属され、かつ同日に竹内が新蟹江店以外の店舗に配属
    #     されている場合、竹内を新蟹江店へ、山岡を竹内がいた店舗へ入れ替えて
    #     配属することを推奨する(2026/9時点の要望。当初は徳重店・極楽店のみが
    #     対象だったが、10/24の実例(竹内=天白植田店)を受けて竹内の勤務可能
    #     店舗全体に拡大)。曜日を問わず全営業日(平日・土日・祝日)に適用する。
    #     他店舗の人員不足解消などでどうしても入れ替えられない場合は、より重い
    #     ペナルティ(W_SHORTAGE等)の方が優先されるため、その場合は入れ替え
    #     なしを許容する。
    ...
    if yamaoka_id_kanie is not None and takeuchi_id_swap is not None:
        for d in business_days:
            ...
            takeuchi_other_terms = [
                ...
                for st in utils.STORES
                if st != "新蟹江店" and (takeuchi_id_swap, d, st) in work_vars
            ]
